...
cBoard.service('dataService', function ($http, $q, updateService) {
    ...
    this.getDataSeries = function (widget,datasource, query, datasetId, chartConfig, callback, reload) {
        chartConfig = angular.copy(chartConfig);
        ...
        linkDataset(datasetId, chartConfig).then(function () {
            ...
            $http.post("dashboard/getAggregateData.do", {
                ...
                //add by hqb 2019-2-27
                data = changeData(widget,chartConfig,data);
    ...
    var castRawData2Series = function (aggData, chartConfig) {
        ...
        for (var i = 0; i < aggData.data.length; i++) {
            ...
            _.each(valueSeries, function (dSeries) {
                ...
                // }
                //update by hqb 2019-6-14 去掉parseFloat，否则若指标是非数值类型(如最大日期),显示不正确
                newData[newGroup][dSeries.name][dSeries.aggType][jk] = aggData.data[i][dSeries.index] //parseFloat(aggData.data[i][dSeries.index]);
            });
        }
        //sort dimension
    ...
    function parserExp(rawExp) {
        ...
        //update by huangqingbo 2019-2-20 ,原正则表达式写法：/(sum|avg|count|max|min|distinct)\("?.*?"?\)/g
        //类似sum(abs(start_hpos-stop_hpos)+0 ) ,这种表达式内部含有“)”的会显示错误
        _.each(evalExp.match(/(sum|avg|count|max|min|distinct|sec_to_time|convert|TIMESTAMPDIFF|date_format)\("?.*?"?\)$/g), function (aggUnit) {
            var aggregate = aggUnit.substring(0, aggUnit.indexOf('('));
            //update by huangqingbo 2019-2-20,将indexOf()方法改为使用lastIndexOf()方法，解决表达式内部若含有括号报错的问题
            //原：var name = aggUnit.substring(aggUnit.indexOf('(') + 1, aggUnit.indexOf(')'));
            //更改如下：
            var name = aggUnit.substring(aggUnit.indexOf('(') + 1, aggUnit.lastIndexOf(')'));
            if (name.match("_#")) {
