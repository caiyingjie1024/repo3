...
cBoard.controller('paramCtrl', function ($scope, $uibModal, $http) {
    ...
    console.log('已传入$scope.param：', $scope.param)
    ...
    $scope.filterValueChange = function (e, valuesIndex, multipleFlag, selectValue) {
        if (valuesIndex == undefined || valuesIndex == null || valuesIndex == '') {
            valuesIndex = 0;
        }
        if (multipleFlag == undefined || multipleFlag == null || multipleFlag == '') {
            multipleFlag = false;
        }
        ...
        if (value == null || value == '') {
            //区间类型的查询条件
            if (multipleFlag) {
                if ($scope.param.values.join("") == '')
                    $scope.param.values = []
                else if ($scope.param.values.length == (valuesIndex + 1))
                    $scope.param.values.pop()
                else
                    $scope.param.values[valuesIndex] = '';
            } else {
                $scope.param.values = [];
            }
        } else {
            $scope.param.values[valuesIndex] = value;
        }
