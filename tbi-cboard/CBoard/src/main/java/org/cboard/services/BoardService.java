...
public class BoardService {
    ...
    public ViewDashboardBoard getBoardData(Long id) {
        ...
        //update by hqb 2019-5-13
        try {
            if (rows != null && !rows.isEmpty()) {
                rows.stream().forEach(o -> {
                    JSONObject row = (JSONObject) o;
                    if (row.containsKey(TYPE) && PARAM.equals(row.getString(TYPE)) && row.containsKey(PARAMS)) {
                        JSONArray params = (JSONArray) row.get(PARAMS);
                        if (params != null && !params.isEmpty()) {
                            params.stream().forEach(param -> {
                                evaluatorByJsonObject((JSONObject) param);
                            });
                        }
                    }
                });
       ...
       // String resultStr = JSONObject.toJSONString(view);
        return view;
    }

    private void evaluatorByJsonObject(JSONObject o) {
        JSONArray values = (JSONArray)o.get(VALUES);
        if(values != null && !values.isEmpty()){
            String test = " test";
            o.replace(VALUES,values.stream().flatMap(v -> getFilterValue((String)v)).collect(Collectors.toCollection(JSONArray::new)));
            return;
        }
    }
    ...
    private Stream<String> getFilterValue(String value) {
        ...
        if(StringUtils.isEmpty(value)
                ...
                || "{loginName}".equals(value)
                || "{userName}".equals(value)
                || "{userRoles}".equals(value)) {
            list.add(value);
        }else {
